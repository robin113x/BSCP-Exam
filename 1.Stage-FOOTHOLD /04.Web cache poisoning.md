...
## 📑 Table of Contents
1. [What is Web Cache Poisoning?](#1️⃣-what-is-web-cache-poisoning)
2. [How Does a Web Cache Work?](#2️⃣-how-does-a-web-cache-work)
3. [Impact of a Web Cache Poisoning Attack](#3️⃣-impact-of-a-web-cache-poisoning-attack)
4. [Constructing a Web Cache Poisoning Attack](#4️⃣-constructing-a-web-cache-poisoning-attack)
5. [Exploiting Web Cache Poisoning Vulnerabilities](#5️⃣-exploiting-web-cache-poisoning-vulnerabilities)
6. [Prevention Techniques](#6️⃣-how-to-prevent-web-cache-poisoning-vulnerabilities)
7. [Lab's](#-lbas)
...
🅰️ **Keyed Cache** ::  A keyed cache is when the cache takes certain request parameters (query strings, headers, cookies, etc.) into account when deciding if a response is the same as a previously cached one.
...
🅱️ **Unkeyed Cache** :: An unkeyed cache is when the cache ignores certain request parameters, so multiple different requests can map to the same cached object.
...
- This means the response was from a CDN’s cache and was cached 253 seconds ago.
...
# Exploiting-design-flaws <a href="https://portswigger.net/web-security/web-cache-poisoning/exploiting-design-flaws">↗️</a>
...
-	Add a cache buster + X-Forwarded-Host: example.com → see it overwrite data.host in initGeoLocate() (DOM-XSS vulnerable).
...
-	Visit the poisoned page in a browser → alert pops.


### Lab: Combining web cache poisoning vulnerabilities
- ***Identify the injection points***
	- In Burp Proxy > HTTP history, load the homepage /.
	-	Use Param Miner → find X-Forwarded-Host and X-Original-URL are supported.
...
Cookie: lang=es
```
- Send → check that the response loads your JSON from exploit server.
- Load it in browser → alert() fires → confirmed.
...
- Keep both caches poisoned:
	- /?localized=1 → cached malicious translation (X-Forwarded-Host).
	- / → cached redirect to Spanish (X-Original-URL).
- Replay both requests every so often to prevent cache expiry.


<br>

# Exploiting cache implementation flaws <a href="https://portswigger.net/web-security/web-cache-poisoning/exploiting-implementation-flaws">↗️</a>

### Lab: Web cache poisoning via an unkeyed query string

1️⃣ Identify a cacheable endpoint

- Pick a page likely to be cached — usually static pages like /, /home, /products.
...
2️⃣ Send two requests to compare
...
3️⃣ Check the response

- If both responses:
	- Return the same body (identical HTML, etc.)
	- And the cache header still says HIT for the second request
    - → the query parameter might be ignored in the cache key.

4️⃣ Confirm with a poison test
...
- Reload / (without the parameter) —
  ***If you still see alert(1) → the parameter was unkeyed and cached.*** 

5️⃣ Automate discovery with Param Miner (Burp Suite)
-	In Burp → Extensions → Install Param Miner.
-	Right-click a request → Guess params.
...
→ Reflected? Cache HIT? If yes → VULNERABLE

```

### Lab: Web cache poisoning via an unkeyed query parameter

##### UTM Parameter : (Urchin Tracking Module parameters) are commonly used for analytics and marketing tracking. The “standard” set used by Google Analytics and many other tools
|Parameter	|Purpose|
|--------------|--------------|
...
-	utm_id — Campaign ID
-	utm_reader — For RSS or newsletter readers
-	utm_name — Alternative campaign name
-	utm_creative — Creative asset identifier
-	utm_social — Social platform name
-	utm_social-type — Type of social media (post, ad, etc.)
...
- Using utm_content=foo;callback=… lets you smuggle extra params (semicolon trick).
...
callback=alert(1)
```
- This poisons the cache → victims load the script and run alert(1).
...
GET /random HTTP/1.1
```
→ The path /random appears in the error response (reflected).
- Inject XSS into the path:
```
GET /random</p><script>alert(1)</script><p>foo HTTP/1.1
```
→ Reflected in the error response.
- In the browser, this doesn’t execute because browsers URL-encode it (e.g. %3Cscript%3E).
- But the cache key normalization means the cache decodes it → your malicious response gets served.
Exploit:
- Use Repeater to poison the cache with the malicious payload above.
- Immediately open the same URL in your browser — now the cached response runs alert(1).
```bash 
...
1.	/login ignores utm_content in cache key → **lets us append extra params.**
2.	/js/localize.js → **CSPP (Client-Side Parameter Pollution) via lang.**
3.	/js/localize.js → **vulnerable to header injection via Origin if cors=1.**
4.	Pragma: x-get-cache-key → **confirms cache key injection possible. if output $$,//,&&**

This lab chains **four behaviors** into a cache poisoning exploit:

1. **Unkeyed parameter**  
   `/login` ignores `utm_content` in the cache key → allows extra parameters to be smuggled.
2. **Client-Side Parameter Pollution (CSPP)**  
   `/js/localize.js` uses `lang` without URL-encoding → vulnerable to client-side injection.
...
```
- Injects a fake header (Content-Length) → body becomes alert(1).
...
1. **Identify cache oracle**
   ...
   - External cache reacts → confirms query strings are keyed.
...
2. **Spot internal cache behavior**
  Even with cache-buster (?cb=123), geolocate.js remains cached.
  Removing the header → poisoned fragment persists.
  ✅ Confirms header is unkeyed by internal cache.
