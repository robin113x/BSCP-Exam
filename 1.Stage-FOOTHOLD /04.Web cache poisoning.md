...
1. /login ignores utm_content in cache key → lets us append extra params.
 /js/localize.js → CSPP (Client-Side Parameter Pollution) via lang.
- /js/localize.js → vulnerable to header injection via Origin if cors=1.
- Pragma: x-get-cache-key → confirms cache key injection possible.
