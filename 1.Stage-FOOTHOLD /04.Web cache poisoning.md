...
1️⃣ Keyed Cache ::  A keyed cache is when the cache takes certain request parameters (query strings, headers, cookies, etc.) into account when deciding if a response is the same as a previously cached one.
...
2️⃣ Unkeyed Cache :: An unkeyed cache is when the cache ignores certain request parameters, so multiple different requests can map to the same cached object.
...
```
---
1️⃣ Check HTTP Response Headers
