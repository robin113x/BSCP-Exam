...
1.  How is a Cookie set by the Server?
The server sets a cookie using the Set-Cookie header in its HTTP response:
```
HTTP/1.1 200 OK
Content-Type: text/html
Set-Cookie: sessionId=abc123; Path=/; HttpOnly; Secure
```
- Key attributes in Set-Cookie:
	- Name=Value: Actual data (e.g., session ID)
...
2. How is a Cookie sent by the Client?
- The browser automatically includes cookies in subsequent HTTP requests via the Cookie header:
```
...
3. What Information is Allowed in a Cookie?
- Allowed:
	- Key-value pairs (e.g., user=John, session=abc123)
	- Metadata like expiration(date/time), domain, path, flags (Secure, HttpOnly)
- Not Allowed:
	- Large data (size limit usually 4KB per cookie)
	- Complex structures (must be string data only)
	- Sensitive data in plain text (must be encrypted if sensitive)
...
# Difference Between Cookies and Caches

| Feature              | **Cookies**                                                       | **Cache**                                                           |
| -------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------- |
| **Purpose**          | Stores **user-specific data** (sessions, preferences, login info) | Stores **static resources** (images, CSS, JS) to improve load speed |
| **Content**          | Small key-value pairs (e.g., `sessionId=abc123`)                  | Files, images, scripts, web pages                                   |
| **Size**             | Limited (\~4KB per cookie)                                        | Much larger (MBs possible)                                          |
| **Storage Location** | Browser storage (sent to server on every request)                 | Browser storage (used locally, not sent to server every time)       |
| **Lifespan**         | Defined by server (expires or session-based)                      | Defined by cache-control headers or user settings                   |
| **Access**           | Can be read by server (and sometimes JS)                          | Only browser uses it; server doesn’t automatically get it           |
| **Use Case**         | Authentication, tracking, personalization                         | Faster page loading, reduced bandwidth usage                        |

## Simple Way to Remember:
- Cookies = "Who you are" (identity & preferences)
- Cache = "What you see" (website content for speed)