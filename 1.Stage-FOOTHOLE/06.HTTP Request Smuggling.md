...
# 2. Checking if the back-end uses Content-Length
```bash
POST / HTTP/1.1
Host: victim.com
Content-Type: application/x-www-form-urlencoded
...
```
- 👉 What happens:
	- Front-end may see Transfer-Encoding: chunked → expects chunked data.
	- But back-end ignores chunked, and instead trusts Content-Length: 6.
	- So, the back-end reads only 6 bytes (0\r\n\r\nX) and may treat the leftover X as the start of the next request.
- ✅ Sign: If the “X” leaks into the next response or causes unexpected behavior, it means the back-end is honoring  Content-Length.