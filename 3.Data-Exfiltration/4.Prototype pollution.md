...
Summary — DOM XSS via client-side prototype pollution

Goal: Use prototype pollution to control a script src and trigger DOM XSS.

Manual steps (quick):

Find pollution vector: Inject into query string: /?__proto__[foo]=bar → confirm Object.prototype.foo === "bar" in DevTools Console.

Identify gadget: Inspect site scripts (Sources). searchLogger.js will append a <script> if config.transport_url exists — a potential gadget if that property is read later.
